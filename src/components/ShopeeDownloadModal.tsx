...
export const ShopeeDownloadModal: React.FC<ShopeeDownloadModalProps> = ({
  ...
  // Auto-calculated prefix based on selected mode & filter (e.g. DELI-balist)
  const autoGeneratedPrefix = useMemo(() => {
    const base = (effectivePrefix || 'balist').trim();
    ...
    return base;
  }, [mode, selectedBrands, selectedCategories, effectivePrefix]);
  ...
  }, [isOpen, autoGeneratedPrefix, comparisonItems]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-stone-200 w-full max-w-3xl overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-4 sm:p-5 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="p-3.5 bg-stone-50 rounded-xl border border-stone-200 space-y-2.5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
              <div>
                ...
                <p className="text-[11px] text-stone-500 mt-0.5">
                  Prefix otomatis menyesuaikan pilihan (contoh: <strong className="text-stone-700 font-mono">DELI-balist</strong>), atau ketik nama sendiri jika diperlukan.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  id="input-custom-prefix-shopee"
                  value={customPrefix}
                  onChange={(e) => setCustomPrefix(e.target.value.replace(/[^a-zA-Z0-9_.-]/g, ''))}
                  placeholder={autoGeneratedPrefix || 'balist'}
                  className="w-36 sm:w-44 bg-white border border-stone-300 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold text-stone-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-center shadow-2xs"
                />
            ...
            <div className="pt-2 border-t border-stone-200/80 flex flex-wrap items-center justify-between gap-1.5 text-[11px]">
              <span className="text-stone-500 flex items-center gap-1 font-medium">
                <Info className="w-3.5 h-3.5 text-stone-400" />
                Pratinjau Nama File Unduhan:
              </span>
              <span className="font-mono font-bold text-emerald-900 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200 text-xs">
                {generateShopeeBalistFilename(customPrefix.trim() || autoGeneratedPrefix)}
              </span>
            </div>
          </div>
        </div>
